...
@click.option("--batch", type=int, default=5, help="Number of pages per run")
@click.pass_context
def ingest_ethdocs_learn_cmd(ctx, batch):
    """Progressive learning from ethereum.org developer docs."""
    from .ethdocs import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_ingested']} pages ingested so far[/dim]")
    with console.status("Fetching from ethereum.org..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Ingested {len(results)} pages:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]No new pages to ingest.[/yellow]")


@ingest.command("eips-learn")
@click.option("--batch", type=int, default=5, help="Number of EIPs per run")
@click.pass_context
def ingest_eips_learn_cmd(ctx, batch):
    """Progressive learning from Ethereum EIP/ERC standards."""
    from .eips import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_ingested']} EIPs ingested so far[/dim]")
    with console.status("Fetching from GitHub..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Ingested {len(results)} EIPs:")
        for r in results:
            console.print(f"  • EIP-{r}")
    else:
        console.print("[yellow]No new EIPs to ingest.[/yellow]")


@ingest.command("defillama-learn")
@click.option("--batch", type=int, default=5, help="Number of protocols per run")
@click.pass_context
def ingest_defillama_learn_cmd(ctx, batch):
    """Progressive learning from DefiLlama (top protocols by TVL)."""
    from .defillama import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_ingested']} items ingested so far[/dim]")
    with console.status("Fetching from DefiLlama..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Ingested {len(results)} items:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]No new protocols to ingest.[/yellow]")


@ingest.command("l2beat-learn")
@click.option("--batch", type=int, default=5, help="Number of L2s per run")
@click.pass_context
def ingest_l2beat_learn_cmd(ctx, batch):
    """Progressive learning from L2Beat (Layer 2 ecosystem)."""
    from .l2beat import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_ingested']} L2s ingested so far[/dim]")
    with console.status("Fetching from L2Beat..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Ingested {len(results)} L2 projects:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]No new L2s to ingest.[/yellow]")


@ingest.command("rekt-learn")
@click.option("--batch", type=int, default=3, help="Number of articles per run")
@click.pass_context
def ingest_rekt_learn_cmd(ctx, batch):
    """Progressive learning from rekt.news (security incidents)."""
    from .rekt import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_ingested']} articles ingested so far[/dim]")
    with console.status("Fetching from rekt.news..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Ingested {len(results)} security articles:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]No new articles to ingest.[/yellow]")


@ingest.command("spellbook-browse")
@click.option("--batch", type=int, default=3, help="Number of protocols to browse")
@click.pass_context
def ingest_spellbook_browse_cmd(ctx, batch):
    """Browse Dune Spellbook SQL models for on-chain data knowledge."""
    from .spellbook_browse import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[dim]Progress: {s['total_browsed']} protocols browsed so far[/dim]")
    with console.status("Browsing Spellbook..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Browsed {len(results)} protocols:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]No new protocols to browse.[/yellow]")


@ingest.command("orchestrate")
@click.option("--batch", type=int, default=5, help="Items per cycle")
@click.pass_context
def ingest_orchestrate_cmd(ctx, batch):
    """Run one orchestrator cycle (auto-decides what to learn)."""
    from .orchestrator import learn, status
    s = status(ctx.obj["base_dir"])
    console.print(f"[cyan]Orchestrator[/cyan] — {s['foundation_coverage']} foundations covered")
    if s.get("missing_foundations"):
        console.print(f"[dim]Missing: {', '.join(s['missing_foundations'][:5])}...[/dim]")
    with console.status("Orchestrator thinking..."):
        results = learn(batch, ctx.obj["base_dir"])
    if results:
        console.print(f"[green]✓[/green] Learned {len(results)} items:")
        for r in results:
            console.print(f"  • {r}")
    else:
        console.print("[yellow]Nothing new to learn this cycle.[/yellow]")
    s2 = status(ctx.obj["base_dir"])
    if s2.get("recent_history"):
        last = s2["recent_history"][-1]
        console.print(f"[dim]Strategy used: {last.get('strategy', '?')}[/dim]")
