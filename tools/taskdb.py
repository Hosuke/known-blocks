...
The DB file is placed at <data_dir>/known-blocks.db where data_dir comes from
...
def migrate_from_json(base_dir: Path) -> dict:
    ...
    migrators = {
        "ethdocs": _migrate_ethdocs,
        "eips": _migrate_eips,
        "defillama": _migrate_defillama,
        "l2beat": _migrate_l2beat,
        "rekt": _migrate_rekt,
        "spellbook": _migrate_spellbook,
    }
    ...
    for source, func in migrators.items():
        ...
        if count > 0:
            ...
            logger.info(f"Migrated {count} tasks from {source}_progress.json")
...
def _migrate_ethdocs(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "ethdocs_progress.json")
    items = data.get("ingested_pages", [])
    for item in items:
        enqueue_task("ethdocs", item, priority=3, base_dir=base_dir)
        # Mark as completed immediately since it was already ingested
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='ethdocs' AND item_key=?",
            (_now(), item),
        )
        db.commit()
    return len(items)
...
def _migrate_eips(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "eips_progress.json")
    items = data.get("ingested_eips", [])
    for item in items:
        key = f"eip-{item}" if isinstance(item, int) else str(item)
        enqueue_task("eips", key, priority=3, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='eips' AND item_key=?",
            (_now(), key),
        )
        db.commit()
    return len(items)
...
def _migrate_defillama(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "defillama_progress.json")
    count = 0
    for item in data.get("ingested_protocols", []):
        enqueue_task("defillama", f"protocol-{item}", priority=2, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='defillama' AND item_key=?",
            (_now(), f"protocol-{item}"),
        )
        db.commit()
        count += 1
    for item in data.get("ingested_chains", []):
        enqueue_task("defillama", f"chain-{item}", priority=2, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='defillama' AND item_key=?",
            (_now(), f"chain-{item}"),
...
def _migrate_l2beat(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "l2beat_progress.json")
    items = data.get("ingested_slugs", [])
    for item in items:
        enqueue_task("l2beat", item, priority=5, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='l2beat' AND item_key=?",
            (_now(), item),
        )
        db.commit()
    return len(items)
...
def _migrate_rekt(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "rekt_progress.json")
    items = data.get("ingested_urls", [])
    for item in items:
        enqueue_task("rekt", item, priority=5, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='rekt' AND item_key=?",
            (_now(), item),
        )
        db.commit()
    return len(items)
...
def _migrate_spellbook(meta_dir: Path, base_dir: Path) -> int:
    data = _read_json(meta_dir / "spellbook_progress.json")
    items = data.get("browsed_protocols", [])
    for item in items:
        enqueue_task("spellbook", item, priority=5, base_dir=base_dir)
        db = get_db(base_dir)
        db.execute(
            "UPDATE tasks SET status='completed', completed_at=? WHERE source='spellbook' AND item_key=?",
            (_now(), item),
        )
        db.commit()
    return len(items)
