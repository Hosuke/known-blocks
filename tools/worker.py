...
def _task_learn(base: Path, source: str, batch_size: int):
    ...
    try:
        if source == "orchestrator":
            from .orchestrator import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] Orchestrator: learned {len(results)} items")
        elif source == "ethdocs":
            from .ethdocs import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] Ethdocs: ingested {len(results)} pages")
        elif source == "eips":
            from .eips import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] EIPs: ingested {len(results)} standards")
        elif source == "defillama":
            from .defillama import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] DefiLlama: ingested {len(results)} protocols")
        elif source == "l2beat":
            from .l2beat import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] L2Beat: ingested {len(results)} L2s")
        elif source == "rekt":
            from .rekt import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] Rekt: ingested {len(results)} articles")
        elif source == "spellbook":
            from .spellbook_browse import learn
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] Spellbook: browsed {len(results)} protocols")
        elif source == "cbeta":
            ...
            results = learn(batch_size=batch_size, base_dir=base)
            logger.info(f"[learn] Wikisource: ingested {len(results)} new works")
        else:
